...
async def get_employee_schedule(
    ...
    employee_id: str,
    filial_id: str,
    api_url: str = "https://back.matrixcrm.ru/api/v1/AI/getEmployeeSchedule",
    api_token: Optional[str] = None
) -> dict:
    """
    Получает расписание сотрудника в указанном филиале.
    """
    params = {
        "employeeId": employee_id,
        "filialId": filial_id
    ...
    try:
        async with httpx.AsyncClient(verify=False) as client: # verify=False для отключения проверки SSL
            logger.info(f"Отправка запроса на {api_url} с параметрами: {params}")
            response = await client.get(api_url, params=params, headers=headers, timeout=20.0)
            response.raise_for_status()
            response_data = response.json()
            logger.info(f"Ответ от API расписания: Code {response_data.get('code')}, Data: {str(response_data.get('data', 'N/A'))[:200]}")
            return response_data
    except httpx.HTTPStatusError as e:
        error_content = e.response.text
        logger.error(f"Ошибка HTTP Status {e.response.status_code} при вызове API расписания: {error_content}", exc_info=True)
        try:
            return {"code": e.response.status_code, "message": f"Ошибка API: {error_content}"}
        except json.JSONDecodeError:
            return {"code": e.response.status_code, "message": f"Ошибка API (не JSON): {error_content}"}
    except httpx.RequestError as e:
        logger.error(f"Ошибка запроса при вызове API расписания: {e}", exc_info=True)
        return {"code": 500, "message": f"Ошибка сети или соединения: {e}"}
    except Exception as e:
        logger.error(f"Неизвестная ошибка при вызове API расписания: {e}", exc_info=True)
        return {"code": 500, "message": f"Внутренняя ошибка: {e}"}